...
const ManageEvent: React.FC = () => {
  ...
  const handleCreateEvent = (formData: any, image: string | null) => {
    ...
    const newEvent: EventData = {
      ...
      seatCategories: formData.seatCategories.map((cat: any) => ({
        ...cat,
        booked: 0,
      })),
  ...
  const handleDeleteEvent = () => {
    if (selectedEventForDelete) {
      ...
      closeAllModals();
      setSuccessMessage(
        `🗑️ Event "${selectedEventForDelete.name}" deleted successfully!`,
      );
      setShowSuccessPopup(true);
      setTimeout(() => setShowSuccessPopup(false), 3000);
    }
  ...
  const handleCancelWithReason = () => {
    if (eventToCancel) {
      ...
      closeAllModals();
      setSuccessMessage(
        `⚠️ Event "${eventToCancel.name}" has been cancelled. Reason: ${cancelReason}`,
      );
      setShowSuccessPopup(true);
      setTimeout(() => setShowSuccessPopup(false), 3000);
    }
  ...
  const handleReactivateEvent = () => {
    if (selectedEventForReactivate) {
      setEvents(
        events.map((e) =>
          e.id === selectedEventForReactivate.id
            ? { ...e, status: "upcoming" }
            : e,
        ),
      );
      closeAllModals();
      setSuccessMessage(
        `✅ Event "${selectedEventForReactivate.name}" has been reactivated!`,
      );
      setShowSuccessPopup(true);
      setTimeout(() => setShowSuccessPopup(false), 3000);
    }
      ...
      Cell: (row: EventData) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">{row.hall}</p>
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      Cell: (row: EventData) => (
        <span className="text-green-600 font-semibold">
          ETB {row.totalRevenue.toLocaleString()}
        </span>
      ),
    },
    {
          ...
          employee={
            selectedEventForDelete
              ? {
                  ...
                  name: selectedEventForDelete.name,
                }
              : null
          }
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl max-w-md w-full p-6">
              ...
              <p className="text-gray-600 mb-4">
                ...
                <strong>{eventToCancel.name}</strong>?
              </p>
              <div className="mb-4">
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl max-w-md w-full p-6">
              ...
              <p className="text-gray-600 mb-6">
                ...
                <strong>{selectedEventForReactivate.name}</strong>?
              </p>
              <div className="flex gap-3">
                <button
                  onClick={() => closeAllModals()}
