...
import ReplyContactReusable from "./ReplyContactReusable";
...
const ContactManagement: React.FC = () => {
  ...
  const filteredMessages = useMemo(() => {
    return messages.filter((m) => {
      ...
      const matchesRecipient =
        filterRecipient === "all" || m.recipientType === filterRecipient;
      return (
        matchesSearch && matchesStatus && matchesCategory && matchesRecipient
      );
    });
  ...
  const stats = useMemo(
    () => ({
      ...
      theaterMessages: messages.filter((m) => m.recipientType === "theater")
        .length,
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: true,
      Cell: (row: ContactMessage) =>
        getRecipientBadge(row.recipientType, row.theaterName),
    },
    {
      Header: "Received",
      accessor: "createdAt",
    ...
    },
    {
      ...
      Cell: (row: ContactMessage) => (
        <div className="flex items-center gap-2">
          ...
          {/* Only show reply button for theater messages */}
          {row.recipientType === "theater" && (
            <ReplyContactReusable
              message={row} // This now works because the component accepts the flexible type
              onReply={handleReply}
            />
          )}
          <button
      ...
      >
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="flex gap-3 pt-4 border-t border-gray-200">
            <button
              onClick={() => setShowModal(false)}
              className="flex-1 px-4 py-2 bg-gradient-to-r from-teal-600 to-emerald-600 text-white rounded-lg hover:from-teal-700 hover:to-emerald-700 transition flex items-center justify-center gap-2"
            >
              Close
            </button>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Manage all incoming customer inquiries
              </p>
            </div>
          </div>
        </div>
