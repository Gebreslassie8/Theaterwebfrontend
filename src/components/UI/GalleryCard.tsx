...
const GalleryCard: React.FC<GalleryCardProps> = ({ image, onClick, index = 0 }) => {
    ...
    const handleLike = (e: React.MouseEvent) => {
        ...
        const likedImages = JSON.parse(localStorage.getItem('liked_gallery_images') || '[]');
        if (!isLiked) {
            if (!likedImages.includes(image.id)) {
                likedImages.push(image.id);
                localStorage.setItem('liked_gallery_images', JSON.stringify(likedImages));
            }
        } else {
            const index = likedImages.indexOf(image.id);
            if (index > -1) {
    ...
    const handleShare = async (e: React.MouseEvent) => {
        ...
        const shareData = {
            title: image.title,
            text: `Check out this amazing theater photo: ${image.title}`,
            url: window.location.href + `?gallery=${image.id}`
        };
    ...
    const handleDownload = async (e: React.MouseEvent) => {
        ...
        try {
            ...
            link.href = url;
            link.download = `theaterhub_${image.title.toLowerCase().replace(/\s+/g, '_')}.jpg`;
