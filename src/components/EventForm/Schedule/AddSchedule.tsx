// src/components/EventForm/Schedule/AddSchedule.tsx
...
interface AddScheduleProps {
    onAdd?: (newEvent: Show) => void;
    onCancel?: () => void;
}

const AddSchedule: React.FC<AddScheduleProps> = ({ onAdd, onCancel }) => {
    ...
    const [formData, setFormData] = useState({
        ...
        contactPhone: '',
        website: '',
        timeSlots: [{ id: generateId(), date: '', startTime: '', endTime: '' }] as TimeSlot[],
    ...
    const getSeatTypesForHall = (hallId: string) => {
        ...
        return hall.seatTypes.map(st => ({
            ...
            name: st,
            price: 0,
            capacity: Math.floor(hall.capacity / hall.seatTypes.length),
    ...
    const handleSubmit = () => {
        if (validateForm()) {
            ...
            setTimeout(() => {
                ...
                setSuccessMessage(`✨ Event "${newEvent.name}" created!`);
            ...
            >
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <select className={`w-full p-2 border rounded-lg ${errors.hall ? 'border-red-500' : 'border-gray-200'}`} value={formData.hall} onChange={e => { handleChange('hall', e.target.value); handleHallChange(e.target.value); }}>
                    ...
                    <div>
                        ...
                        <textarea rows={4} className={`w-full p-2 border rounded-lg ${errors.description ? 'border-red-500' : 'border-gray-200'}`} value={formData.description} onChange={e => handleChange('description', e.target.value)} />
                    ...
                    <div>
                        ...
                        <div className="border-2 border-dashed rounded-xl p-4 text-center">
                            <input type="file" accept="image/*" onChange={handleImageUpload} className="hidden" id="upload-add" />
                            <label htmlFor="upload-add" className="cursor-pointer block">
                ...
                <div className="border-t p-4 bg-gray-50 flex justify-end gap-3 shrink-0">
                    ...
                    <ReusableButton onClick={handleSubmit} variant="success" disabled={saving} loading={saving} icon={Save}>Create Event</ReusableButton>
                </div>
            </motion.div>
